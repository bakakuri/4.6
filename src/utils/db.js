...
export const getFriends = async (userId) => {
  const { data } = await supabase.from('friends')
    .select('*, user:user_id(id,username,xp,streak,photo_url), friend:friend_id(id,username,xp,streak,photo_url)')
    .or(`user_id.eq.${userId},friend_id.eq.${userId}`)
    .eq('status', 'accepted')
  return (data || []).map(f => f.user_id === userId ? f.friend : f.user)
}
