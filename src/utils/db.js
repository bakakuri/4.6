...
export const getFriends = async (userId) => {
  ...
  const friendIds = data.map(r => r.user_id === userId ? r.friend_id : r.user_id)
  const { data: profs } = await supabase.from('profiles')
    .select('id,username,xp,streak,photo_url').in('id', friendIds)
  return profs || []
}
export const getPendingFriends = async (userId) => {
  ...
  const { data: profs } = await supabase.from('profiles')
    .select('id,username,xp,streak,photo_url').in('id', ids)
  const profMap = Object.fromEntries((profs||[]).map(p => [p.id, p]))
  return data.map(r => ({ id: r.id, user: profMap[r.user_id] || { id: r.user_id, username: 'Unknown' } }))
}
export const sendFriendRequest = async (userId, friendId) => {
  const { data, error } = await supabase.from('friends')
    .insert({ user_id: userId, friend_id: friendId }).select().single()
  if (error) throw error; return data
}
export const respondFriendRequest = async (id, accept) => {
  await supabase.from('friends').update({ status: accept ? 'accepted' : 'declined' }).eq('id', id)
}
...
export const getFriendStatus = async (userId, friendId) => {
  const { data } = await supabase.from('friends').select('id,status,user_id')
    .or(`and(user_id.eq.${userId},friend_id.eq.${friendId}),and(user_id.eq.${friendId},friend_id.eq.${userId})`)
    .single()
  return data
}
...
// ── Custom Words ──────────────────────────────────────────────
export const getCustomWords = async (userId, lang) => {
  const { data } = await supabase.from('custom_words')
    .select('*').eq('user_id', userId).eq('lang', lang)
    .order('created_at', { ascending: false })
  return data || []
}
export const addCustomWord = async (userId, lang, word, translation, phonetic, example) => {
  const { data, error } = await supabase.from('custom_words')
    .insert({ user_id: userId, lang, word, translation, phonetic, example }).select().single()
  if (error) throw error; return data
}
export const deleteCustomWord = async (id) =>
  supabase.from('custom_words').delete().eq('id', id)

// ── Weak Words (სუსტი სიტყვები) ──────────────────────────────
export const getWeakWordIds = async (userId, lang) => {
  const { data } = await supabase.from('word_progress')
    .select('word_id, mastery, wrong_count')
    .eq('user_id', userId).eq('lang', lang)
    .lt('mastery', 75)
    .order('mastery', { ascending: true }).limit(50)
  return (data || []).map(d => d.word_id)
}
export const incrementWrongCount = async (userId, lang, wordId) => {
  const { data } = await supabase.from('word_progress')
    .select('wrong_count').eq('user_id', userId).eq('lang', lang).eq('word_id', wordId).single()
  await supabase.from('word_progress')
    .update({ wrong_count: ((data?.wrong_count) || 0) + 1 })
    .eq('user_id', userId).eq('lang', lang).eq('word_id', wordId)
}

// ── Daily Activity (სტატისტიკის გრაფიკი) ─────────────────────
export const getDailyActivity = async (userId, lang, days = 14) => {
  const since = new Date(); since.setDate(since.getDate() - days)
  const { data } = await supabase.from('word_progress')
    .select('updated_at').eq('user_id', userId).eq('lang', lang)
    .gte('updated_at', since.toISOString())
  const map = {}
  ;(data || []).forEach(r => { const d = r.updated_at.slice(0,10); map[d] = (map[d]||0)+1 })
  const result = []
  for (let i = days-1; i >= 0; i--) {
    const d = new Date(); d.setDate(d.getDate()-i)
    const k = d.toISOString().slice(0,10)
    result.push({ date: k, count: map[k]||0, label: `${d.getDate()}/${d.getMonth()+1}` })
  }
  return result
}
...
export const getPublicProfile = async (userId) => {
  const { data } = await supabase.from('profiles')
    .select('id,username,xp,streak,photo_url,current_lang,created_at,achievements,sessions')
    .eq('id', userId).single()
  return data
}

// ── Practice Queue ────────────────────────────────────────────
export const getPracticeQueue = async (userId, lang) => {
  const { data } = await supabase.from('practice_queue')
    .select('word_id').eq('user_id', userId).eq('lang', lang)
    .order('added_at', { ascending: true })
  return (data || []).map(r => r.word_id)
}

export const getPracticeQueueCount = async (userId, lang) => {
  const { count } = await supabase.from('practice_queue')
    .select('id', { count:'exact', head:true })
    .eq('user_id', userId).eq('lang', lang)
  return count || 0
}
...
export const addToPracticeQueue = async (userId, lang, wordId) => {
  const { error } = await supabase.from('practice_queue')
    .insert({ user_id: userId, lang, word_id: wordId })
  return !error
}

export const removeFromPracticeQueue = async (userId, lang, wordId) => {
  await supabase.from('practice_queue')
    .delete().eq('user_id', userId).eq('lang', lang).eq('word_id', wordId)
}

export const clearPracticeQueue = async (userId, lang) => {
  await supabase.from('practice_queue')
    .delete().eq('user_id', userId).eq('lang', lang)
}
