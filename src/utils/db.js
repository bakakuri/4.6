/**
 * db.js – ყველა Supabase ოპერაცია
 * ყველა ფუნქცია async და გამოიყენება user.id (UUID)-ით
 */
...
/** Upsert one word's mastery (fire-and-forget friendly) */
...
/**
 * Pure local computation – pass the already-loaded progMap.
 * Used by FlashcardScreen for instant next-card (no extra DB call).
 */
export const nextCardFromProgress = (progMap, lang) => {
  const ws = allWords(lang)
  // 1. unseen words first
  const unseen = ws.filter(w => !progMap[w.id])
  if (unseen.length) return unseen[0]
  // 2. lowest mastery, oldest ts
  const candidates = ws
...
// ── Session & Activity (RPC – atomic) ─────────────────────────

export const bumpSession = (userId) =>
  supabase.rpc('inc_session', { uid: userId })

export const bumpActivity = (userId) =>
  supabase.rpc('inc_activity', { uid: userId, dow: new Date().getDay(), wstart: weekStart() })

// ── Chat analytics (RPC – atomic) ────────────────────────────

export const recordCorrect = (userId) =>
  supabase.rpc('inc_chat', { uid: userId, correct: true })

export const recordAnswer = (userId) =>
  supabase.rpc('inc_chat', { uid: userId, correct: false })
