...
function Inner() {
  ...
  // ── Font size & notifications on mount ───────────────────────
  useEffect(() => {
    ...
    if (Notification.permission === 'granted' && localStorage.getItem('lm_notif')) {
      const lastStudied = localStorage.getItem('lm_last_studied')
      const today = new Date().toDateString()
      if (lastStudied !== today) {
        setTimeout(() => {
          new Notification('LinguaMaster 🌍', {
            body: 'დღეს ჯერ არ გისწავლია! გამოიყენე 5 წუთი ახალი სიტყვებისთვის.',
            icon: '/icon.svg', tag: 'daily-reminder'
          })
        }, 30000) // 30 sec after open
      }
    }
  }, [])
