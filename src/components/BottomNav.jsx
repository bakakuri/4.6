...
export default function BottomNav({ page, onNav, isAdmin }) {
  ...
  return (
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(7,9,26,0.97)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderTop: `1px solid ${C.bdL}`, display: 'flex', justifyContent: 'space-around', padding: 'max(6px, env(safe-area-inset-bottom)) 0 6px', fontFamily: "'Inter',system-ui,sans-serif" }}>
      {tabs.map(t => (
        <button key={t.id} onClick={() => onNav(t.id)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0', fontFamily: 'inherit' }}>
