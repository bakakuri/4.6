...
export default function AuthScreen() {
  ...
  const inp = (k, type, ph) => (
    <input key={k} type={type} placeholder={ph} value={form[k]} onChange={set(k)}
      ...
      style={{ width: '100%', background: C.card3, border: `1px solid ${C.bdL}`, borderRadius: 12,
               padding: '14px 16px', color: C.t, fontSize: 15, outline: 'none',
  ...
  const handle = async () => {
    ...
    setBusy(true)
    // Supabase auth uses email internally; user sees only username
    ...
    if (mode === 'register') {
      ...
      if (error) { setErr(error.message === 'User already registered' ? 'მომხმარებელი უკვე არსებობს' : error.message); setBusy(false); return }
    } else {
      const { error } = await supabase.auth.signInWithPassword({ email, password })
      if (error) { setErr('არასწორი მომხმარებელი ან პაროლი'); setBusy(false); return }
    }
    // App.jsx picks up the session change via onAuthStateChange
    setBusy(false)
  }

  return (
    <div style={{ minHeight: '100vh', background: `linear-gradient(135deg,${C.bg} 0%,#080d2a 50%,${C.bg} 100%)`,
          ...
          {['login', 'register'].map(m => (
            <button key={m} onClick={() => { setMode(m); setErr('') }}
              style={{ flex: 1, padding: '9px 0', border: 'none', borderRadius: 8, cursor: 'pointer',
                       ...
                       background: mode === m ? C.a : 'transparent', color: mode === m ? '#fff' : C.ts }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <button onClick={handle} disabled={busy}
            style={{ width: '100%', padding: '14px 0', background: busy ? C.card3 : `linear-gradient(135deg,${C.a},${C.p})`,
                     border: 'none', borderRadius: 12, color: busy ? C.ts : '#fff', fontSize: 16,
                     fontWeight: 700, cursor: busy ? 'default' : 'pointer', fontFamily: 'inherit',
                     boxShadow: busy ? 'none' : `0 4px 20px ${C.aG}`, transition: 'all .2s' }}>
