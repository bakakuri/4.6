...
function MultiChoice({ user, lang, onExit }) {
  ...
  const next = useCallback(() => {
    ...
    setQ(word); setOpts([word.t, ...wrong].sort(() => Math.random() - 0.5)); setSel(null)
  }, [ws])
  ...
  const pick = (opt) => {
    if (sel) return
    setSel(opt); recordAnswer(user.id)
    if (opt === q.t) { setScore(s => s + 1); recordCorrect(user.id) }
    setTotal(v => v + 1)
...
function FillBlank({ user, lang, onExit }) {
  ...
  if (!q) return null
  const exitBtn = { background: C.card3, border: `1px solid ${C.bdL}`, borderRadius: 8, padding: '5px 10px', color: C.ts, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }
  const inpStyle = { width: '100%', background: res === true ? `${C.g}22` : res === false ? `${C.r}22` : C.card3, border: `1px solid ${res === true ? C.g : res === false ? C.r : C.bdL}`, borderRadius: 12, padding: '14px 16px', color: C.t, fontSize: 16, outline: 'none', boxSizing: 'border-box', marginBottom: 12, fontFamily: 'inherit' }
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        <span style={{ color: C.ts, fontSize: 13 }}>სწორი: {score}/{total}</span>
      ...
      <input value={inp} onChange={e => setInp(e.target.value)} onKeyDown={e => e.key === 'Enter' && check()} placeholder={`${LANG[lang].name}-ად ჩაწერე...`} disabled={res !== null} style={inpStyle} />
      {res !== null && <div style={{ textAlign: 'center', fontSize: 15, fontWeight: 700, color: res ? C.g : C.r, marginBottom: 8 }}>{res ? '✅ სწორია!' : `❌ სწორი: ${q.w}`}</div>}
      <button onClick={check} disabled={res !== null || !inp.trim()} style={{ width: '100%', padding: '13px 0', background: `linear-gradient(135deg,${C.a},${C.p})`, border: 'none', borderRadius: 12, color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', opacity: (res !== null || !inp.trim()) ? 0.4 : 1, fontFamily: 'inherit' }}>შემოწმება ✓</button>
    </div>
  )
}
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        {cards.map(card => {
          ...
          return (
            <button key={card.id} onClick={() => flip(card)} style={{ height: 70, borderRadius: 12, cursor: 'pointer', fontWeight: 700, transition: 'all 0.3s', fontFamily: 'inherit', padding: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', lineHeight: 1.3,
              ...
              color: isOpen ? C.t : C.bg, fontSize: isOpen ? 12 : 20 }}>
              {isOpen ? card.word : '?'}
            </button>
          )
        })}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button onClick={check} disabled={!ans.length || res !== null} style={{ flex: 2, padding: '11px 0', background: `linear-gradient(135deg,${C.a},${C.p})`, border: 'none', borderRadius: 10, color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', opacity: (!ans.length || res !== null) ? 0.4 : 1, fontFamily: 'inherit' }}>შემოწმება ✓</button>
      </div>
    </div>
  )
}
...
function Listening({ user, lang, onExit }) {
  ...
  return (
    <div>
      ...
      <input value={inp} onChange={e => setInp(e.target.value)} onKeyDown={e => e.key === 'Enter' && check()} placeholder="ჩაწერე გაგონილი სიტყვა..." disabled={res !== null} style={inpStyle} />
