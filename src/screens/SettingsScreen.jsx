...
export default function SettingsScreen({ user, lang, onLangChange, onLogout }) {
  ...
  const clearData = () => {
    const keys = Object.keys(localStorage).filter(k =>
      k.startsWith('prog_') || k.startsWith('act_') ||
      k.startsWith('sess_') || k.startsWith('streak_') ||
      k.startsWith('chat_') || k === 'chat_messages'
    );
  ...
  return (
    <div style={{ padding: '16px 16px 20px', fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ ...gls({ padding: 16 }), marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {Object.entries(LANG).map(([key, { name, flag, ka }]) => (
            <button key={key} onClick={() => onLangChange(key)} style={{ padding: '12px 14px', background: lang === key ? `linear-gradient(135deg,${C.aG},rgba(168,85,247,0.1))` : C.card3, border: `1px solid ${lang === key ? C.a : 'transparent'}`, borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'inherit' }}>
      ...
      <div style={{ ...gls({ padding: 16 }), marginBottom: 12, border: `1px solid ${C.r}44` }}>
        ...
        {!confirm ? (
          <button onClick={() => setConfirm(true)} style={{ width: '100%', padding: '13px 0', background: `${C.r}22`, border: `1px solid ${C.r}55`, borderRadius: 12, color: C.r, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>🗑️ პროგრესის გაწმენდა</button>
        ) : (
          <div>
            <div style={{ color: C.ts, fontSize: 13, marginBottom: 12, textAlign: 'center' }}>დარწმუნებული ხარ? ყველა პროგრესი წაიშლება!</div>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={clearData} style={{ flex: 1, padding: '12px 0', background: C.r, border: 'none', borderRadius: 10, color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>წაშლა ✓</button>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Logout */}
      <button onClick={onLogout} style={{ width: '100%', padding: '14px 0', background: 'transparent', border: `1px solid ${C.bdL}`, borderRadius: 12, color: C.ts, fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
        🚪 გამოსვლა
      </button>
    </div>
  );
