        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          ...
          <div style={{ display:'flex', gap:16 }}>
            <div style={{ textAlign:'center' }}>
              <div style={{ color:C.a, fontWeight:800, fontSize:20 }}>{myScore}</div>
              <div style={{ color:C.ts, fontSize:9 }}>შენ</div>
            </div>
            <div style={{ color:C.bdL, fontSize:20, paddingTop:2 }}>⚔️</div>
            <div style={{ textAlign:'center' }}>
              <div style={{ color:C.o, fontWeight:800, fontSize:20 }}>{opScore}</div>
    ...
    <div style={S}>
      ...
      {users.length === 0 ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {users.map(u => (
            <div key={u.id} style={{ ...gls({ padding:'12px 14px' }),
