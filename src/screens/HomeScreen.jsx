...
export default function HomeScreen({ user, lang, onNav }) {
  ...
  if (loading) return (
    <div style={{ padding: 24, display: 'flex', justifyContent: 'center', paddingTop: 60 }}>
      <div style={{ color: C.ts, fontSize: 14 }}>იტვირთება...</div>
    </div>
  )
            ...
            return (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{ width: '100%', height: 44, background: C.card3, borderRadius: 6, display: 'flex', alignItems: 'flex-end', overflow: 'hidden' }}>
                  <div style={{ width: '100%', height: `${v}%`, background: isT ? `linear-gradient(180deg,${C.a},${C.p})` : C.a,
                                opacity: isT ? 1 : .5, borderRadius: 6, minHeight: v > 0 ? 4 : 0, transition: 'height .4s' }} />
                </div>
