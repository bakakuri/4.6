...
import {
  getProgress,
  saveProgress,
  nextCardFromProgress,
  bumpSession,
  bumpActivity,
  addXP,
  getWeakWordIds,
  addToPracticeQueue,
  removeFromPracticeQueue,
} from '../utils/db.js'
...
export default function FlashcardScreen({ user, lang }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const prog = (await getProgress(user.id, lang)) || {}
        if (cancelled) return
        ...
        if (!cancelled) {
          setProgress({})
          setCard(nextCardFromProgress({}, lang, weakMode && weakIds.length ? weakIds : null))
        }
      } finally {
  ...
  // Cleanup floating XP timer on unmount
  useEffect(() => {
    return () => clearTimeout(xpTimer.current)
  }, [])
  ...
  useEffect(() => {
    if (card?.w && lc?.code) {
      ...
      if (card.ex) {
        preload(card.ex, lc.code)
      }

      return () => clearTimeout(t)
    }
  }, [card?.id, card?.ex, lc?.code])

  const answer = (mastery) => {
    if (!card || !progress) return

    const xpMap = {
      100: XP_REWARD.flashcard,
      75: Math.round(XP_REWARD.flashcard * 0.5),
      50: Math.round(XP_REWARD.flashcard * 0.25),
      25: 0,
    }
    ...
    setProgress(newProg)
    setCard(nextCardFromProgress(newProg, lang, weakMode && weakIds.length ? weakIds : null))
    setFlipped(false)
    setCount((n) => n + 1)
  ...
  const learned = words.filter((w) => (progress?.[w.id]?.mastery || 0) >= 100)
    ...
    return (
      ...
      >
        <div style={{ textAlign: 'center', paddingTop: 20 }}>
          ...
          <div style={{ color: C.t, fontWeight: 800, fontSize: 20, marginTop: 10 }}>
            ყველა სიტყვა ნასწავლია!
          </div>
          <div style={{ color: C.ts, fontSize: 14, marginTop: 6 }}>
            ბრავო! ახლა გააგრძელე ახალ დონეზე.
          </div>
        </div>
        ...
        {learned.length > 0 && (
          <div style={{ width: '100%', maxWidth: 380 }}>
            <div style={{ color: C.t, fontWeight: 700, fontSize: 15, marginBottom: 10 }}>
              ✅ ნასწავლი სიტყვები
            </div>
            {learned.map((w) => (
              <div
                key={w.id}
                style={{
                  ...gls({ padding: '12px 14px', marginBottom: 8 }),
                  display: 'flex',
  ...
  return (
    <div className="page-enter" style={{ padding: '16px 16px 20px', fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button
            ...
            style={{
              background: weakMode ? C.r + '22' : C.card3,
              border: '1px solid ' + (weakMode ? C.r + '66' : C.bdL),
              borderRadius: 8,
              padding: '3px 8px',
              color: weakMode ? C.r : C.ts,
              fontSize: 10,
              fontWeight: 700,
              cursor: 'pointer',
              fontFamily: 'inherit',
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ color: C.g, fontSize: 13 }}>
      ...
      <div
        ...
        style={{ cursor: 'pointer', perspective: 1000, marginBottom: 16 }}
      >
        <div
          style={{
            position: 'relative',
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              {cardLevel && (
                <span
                  style={{
                    background: LEVEL_COLORS[cardLevel] || C.a,
                    borderRadius: 6,
                    padding: '3px 10px',
                    fontSize: 11,
                    color: '#fff',
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 38, fontWeight: 900, color: C.t, letterSpacing: -1, marginBottom: 8 }}>
                {card.w}
              </div>
              <div style={{ color: C.a, fontSize: 15, marginBottom: 16 }}>{card.ph}</div>
              <div style={{ borderTop: `1px solid ${C.bdL}`, paddingTop: 14 }}>
                <div style={{ color: C.ts, fontSize: 14, fontStyle: 'italic', marginBottom: 6 }}>
                  "{card.ex}"
                </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button
                  onClick={(e) => togglePractice(e, card.id)}
                  disabled={pqBusy}
                  title={practiceIds.has(card.id) ? 'სამეცადინოდან ამოღება' : 'სამეცადინოში დამატება'}
                  className="tap"
                  style={{
                    background: practiceIds.has(card.id) ? `${C.o}33` : C.card3,
                    border: `1px solid ${practiceIds.has(card.id) ? C.o + '88' : C.bdL}`,
                    borderRadius: 10,
                    padding: '8px 11px',
                    color: practiceIds.has(card.id) ? C.o : C.ts,
                    fontSize: 18,
                    cursor: 'pointer',
                    opacity: pqBusy ? 0.5 : 1,
                  }}
                >
          ...
          <div
            style={{
              ...
              flexDirection: 'column',
              justifyContent: 'space-between',
              background: `linear-gradient(160deg,${C.card3},${C.card4})`,
            }}
          >
            <div style={{ textAlign: 'center', paddingTop: 8 }}>
              ...
              <div style={{ borderTop: `1px solid ${C.bdL}`, paddingTop: 14 }}>
                <div style={{ color: C.ts, fontSize: 13, marginBottom: 6 }}>
                  "{card.ex}"
                </div>
                <div style={{ color: C.t, fontSize: 14, fontWeight: 600 }}>
                  "{card.ext}"
                </div>
              </div>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8 }}>
        {[
          ...
          <button
            key={btn.label}
            onClick={() => answer(btn.mastery)}
            className="tap"
            style={{
              padding: '12px 4px',
              background: `${btn.bg}22`,
              border: `1px solid ${btn.bg}66`,
              borderRadius: 12,
              cursor: 'pointer',
              display: 'flex',
              flexDirection: 'column',
      ...
      {learned.length > 0 && (
        <div style={{ marginTop: 20 }}>
          <div style={{ color: C.t, fontWeight: 700, fontSize: 14, marginBottom: 10 }}>
            ✅ ნასწავლი ({learned.length})
          </div>
